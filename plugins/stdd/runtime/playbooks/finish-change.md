...
has authorized that action. A matching `## Permissions` entry in
`.stdd/policy.md` is that authorization, but only once this session has
verified the entry's condition and said what it verified; an unverifiable
condition leaves the rule exactly as it stands.
