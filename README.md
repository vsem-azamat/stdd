...
The CLI is decomposed as an acyclic, flat `cli/*.mjs` module graph. The
`cli/stdd.mjs` executable owns argument order and dispatch only; lower modules
must be import-pure, must not read `process.argv` at module load, and may depend
only toward lower filesystem/config/state layers. Flat files are required
because the universal plugin mirrors the CLI runtime exactly. Refactor slices
move one dependency seam at a time without changing command output, exit codes,
or generated bytes; each slice rebuilds the plugin and keeps its committed
runtime mirror free of stale files.
