...
<!-- generated by stdd v0.10.0 — do not edit, re-run `stdd init` -->

## STDD

This repository follows `.stdd/method.md`. Route read-only current-state
factual or diagnostic questions directly to Investigation with `/stdd-investigation`.
Route opinions, future behavior, and hypothetical implementation approaches
directly to Brainstorming with `/stdd-brainstorming`.
...
persist a work artifact or modify the repository, using `/stdd-start-change`.
...
refactors), invoke `/stdd-start-change`; use `/stdd-implement` for the
docs/red/green/verify slice and `/stdd-finish-change` to close it.
