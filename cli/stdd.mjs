...
import { hasLocalStddBinary, installSessionHook, installStopHook } from "./claude-hooks.mjs";
import {
	appendDeferred,
	compileCapabilities,
	DEFAULT_CONFIG,
...
function isGeneratedPrePushHook(content) {
	return (
		...
		/^npm exec --offline --package=@stdd\/cli@\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)? -- stdd check \. \|\| exit 1\n$/.test(
			content.slice(PRE_PUSH_HEADER.length),
		)
	);
}
...
function readManifestFiles(targetDir) {
	...
	if (!fs.existsSync(manifestPath)) return Object.create(null);
	let parsed;
	try {
		parsed = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
	} catch (err) {
		fail(`.stdd/manifest.json is not valid JSON: ${err.message}`);
	}
	if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
		fail(".stdd/manifest.json must contain a JSON object");
	}
	const files = parsed.files ?? Object.create(null);
	if (typeof files !== "object" || files === null || Array.isArray(files)) {
		fail(".stdd/manifest.json files must be an object");
	}
	for (const file of Object.keys(files)) {
		try {
			resolveWritableRepoPath(targetDir, file, `manifest path ${JSON.stringify(file)}`);
		} catch (err) {
			fail(`.stdd/manifest.json has an unsafe path: ${err.message}`);
		}
	}
	return files;
}
...
function init(targetDir, opts) {
	...
	const writeGenerated = (relPath, content) => {
		...
		try {
			filePath = resolveWritableRepoPath(
				targetDir,
				relPath,
				`generated path ${JSON.stringify(relPath)}`,
			);
		} catch (err) {
			fail(err.message);
		}
		fs.mkdirSync(path.dirname(filePath), { recursive: true });
		// Re-check after mkdir: no concurrent symlink substitution may redirect
		// the final write outside the repository.
		filePath = resolveWritableRepoPath(targetDir, relPath, `generated path ${JSON.stringify(relPath)}`);
	...
	if (tools.includes("codex")) {
		const lines = [
			...
			"",
			"If `stdd` is not on PATH, run the project-local package offline",
			`(\`pnpm exec stdd\`, \`${NPM_RUNNER}\`) — never resolve`,
			"the unrelated unscoped registry package `stdd`.",
		...
		const agentsPath = resolveWritableRepoPath(targetDir, "AGENTS.md", "AGENTS.md path");
	...
	if (hooks) {
		...
		if (fs.existsSync(hookPath)) {
			...
			const updated = prePushHook(NPM_RUNNER);
			...
			fs.writeFileSync(hookPath, prePushHook(NPM_RUNNER), { mode: 0o755 });
			console.log(
	...
	if (sessionHook) installSessionHook(targetDir, NPM_RUNNER);
	if (stopHook) installStopHook(targetDir, NPM_RUNNER);
	...
	fs.writeFileSync(
		resolveWritableRepoPath(targetDir, ".stdd/manifest.json", "manifest path"),
	...
	for (const [file, hash] of Object.entries(oldFiles)) {
		...
		const filePath = resolveWritableRepoPath(targetDir, file, `manifest path ${JSON.stringify(file)}`);
	...
	if (fs.existsSync(manifestPath)) {
		...
		for (const [file, hash] of Object.entries(manifest.files ?? {})) {
			...
			try {
				...
				stale.push({ file, message: `unsafe manifest entry — ${err.message}` });
				continue;
			}
...
function doctor(targetDir, readinessOnly = false) {
	...
	try {
		...
		// no Claude settings — no session automation to assess
	}
...
function removeReviewBrief(request) {
	...
	if (
		...
		!path.basename(dir).startsWith("stdd-review-") ||
	...
	try {
		...
		// Never recursively delete a ledger-named directory. If something else
		// appeared there, remove only the exact brief we created.
	}
	return true;
}
...
function reviewCleanup(cwd) {
	...
	const open = events.filter(
		(e) => e.event === "review-request" && e.via === "subagent" && !reviewRequestAnswered(events, e.id),
	);
	let removed = 0;
	for (const request of open) {
		if (removeReviewBrief(request)) removed++;
		appendLedger(cwd, {
			event: "review-cancelled",
			request: request.id,
			via: request.via,
			reason: "cancelled by stdd review --cleanup",
		});
	}
	console.log(`stdd review: cleaned ${removed} private brief(s), cancelled ${open.length} request(s)`);
}
...
function gateReasons(cwd, inputs = null) {
	...
	const events = inputs?.ledgerPath
		? fs.existsSync(inputs.ledgerPath)
			? parseLedger(fs.readFileSync(inputs.ledgerPath, "utf8")).filter((e) => e.branch === branch)
			: []
		: loadLedger(cwd, branch);
	const reasons = [];
...
if (command === "review") {
	...
	if (cleanup) {
		if (result !== null || via !== null || force) {
			fail("--cleanup cancels open subagent requests and cannot be combined with dispatch flags");
		}
		reviewCleanup(cwd);
	} else if (result !== null) {
	...
	default: {
		...
		console.log(
			"Usage: stdd <init|configure|check|check-pr|evidence|doctor|status|ci|docs|red|verify|note|defer|slice|scope|review|stop-hook> " +
				"[dir|pr-body-file|pr] [--tools claude,codex] [--ci github] [--hooks] " +
