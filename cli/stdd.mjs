...
// Keep every removed or renamed shipped playbook basename here permanently.
// A prior manifest can prove bytes, but only this cumulative registry can prove
// that STDD ever owned a direct child of .stdd/playbooks/.
const RETIRED_PLAYBOOK_FILES = new Set(["retired-routing.md"]);
const KNOWN_MANAGED_PLAYBOOK_FILES = new Set([...SHIPPED_PLAYBOOK_FILES, ...RETIRED_PLAYBOOK_FILES]);
...
function checkoutSnapshot(cwd) {
	...
	for (const file of STATE_EXEMPT) delete dirty[file];
...
function captureReviewMaterial(cwd, baseRef, strict = false) {
	...
	for (const p of REVIEW_EXEMPT) delete reviewDirty[p];
	if (strict) {
...
function settleReviewPrivateDirectory(
	...
	try {
		...
		const openedTemp = fs.fstatSync(tempDescriptor, { bigint: true });
		const heldTemp = `/proc/self/fd/${tempDescriptor}`;
		if (
		...
		if (fs.realpathSync(heldDir) !== realDir) return false;
		const names = fs.readdirSync(heldDir);
		if (names.some((name) => !isReviewOwnedArtifact(request, name))) return false;
		if (
		...
		if (expectedHash !== null) {
			...
			const observedHash = hashReviewDescriptor(brief.descriptor);
			if (
		...
		for (const artifact of artifacts) {
			...
			if (
				!sameReviewFileObservation(artifact.opened, held) ||
				!sameReviewFileObservation(held, atPath)
			) {
				return false;
			}
		}

		quarantine = openReviewQuarantine(realTempRoot, tempDescriptor, heldTemp);
		if (quarantine === null) return false;
		const sourceDir = path.join(heldTemp, path.basename(realDir));
		const targetDir = path.join(
			quarantine.heldQuarantine,
			`review-${request.id}-${randomBytes(16).toString("hex")}.tombstone`,
		);
		const beforeMove = fs.fstatSync(dirDescriptor, { bigint: true });
		...
		fs.fsyncSync(quarantine.quarantineDescriptor);
		fs.fsyncSync(quarantine.tempDescriptor);
		const afterMove = fs.fstatSync(dirDescriptor, { bigint: true });
		...
		closeReviewQuarantine(quarantine);
	}
}
	...
	const runner =
		via === "codex"
			...
			: {
					...
					label: "-p --permission-mode plan (headless, read-only)",
