...
function scanRepo(targetDir, config) {
	...
	if (fs.existsSync(skillsDir)) {
		for (const name of fs.readdirSync(skillsDir)) {
			...
			if (m && m[1] !== VERSION) {
				stale.push({ file: `.claude/skills/${name}/SKILL.md`, version: m[1] });
			}
		}
	}

	return { artifacts, canonicalFiles, temporal, stale };
}
...
function check(targetDir) {
	...
	const violations = [
		...
		...stale.map(
			(s) => `${s.file}: generated by stdd v${s.version}, CLI is v${VERSION} — re-run stdd init`,
		),
	];
