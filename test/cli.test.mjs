...
test("init installs native Claude and Codex skills plus minimal instruction files", async () => {
	...
	assert.match(claudeSkill, /adaptive conversational stance/i);
	assert.match(claudeSkill, /at most one per turn/i);
	assert.match(claudeSkill, /question batch only when the user requests/i);
	assert.match(claudeSkill, /No action:[\s\S]*Defer:[\s\S]*Act now:/i);
	assert.match(
		claudeSkill,
		/Act now:[\s\S]*invoke `stdd-start-change` if no action boundary[\s\S]*Within an active change,[\s\S]*Planning or Implement/i,
	);
	assert.match(claudeSkill, /do not manufacture alternatives/i);
...
test("init renders the five conversational routes in each host's native syntax", async () => {
	const dir = tmpRepo();
	const res = await run(["init", dir, "--tools", "claude,codex,pi"]);
	assert.equal(res.code, 0, res.stdout + res.stderr);
	for (const [relative, invocations] of [
		[
			[".stdd", "CLAUDE-snippet.md"],
			["/stdd-investigation", "/stdd-brainstorming", "/stdd-start-change"],
		],
		[
			[".stdd", "AGENTS-snippet.md"],
			["$stdd-investigation", "$stdd-brainstorming", "$stdd-start-change"],
		],
		[
			[".stdd", "PI-snippet.md"],
			["/skill:stdd-investigation", "/skill:stdd-brainstorming", "/skill:stdd-start-change"],
		],
	]) {
		const router = fs.readFileSync(path.join(dir, ...relative), "utf8");
		for (const invocation of invocations) assert.ok(router.includes(invocation), invocation);
		assert.match(router, /unknown current facts materially affect future design/i);
		assert.match(router, /explicit intent.*persist.*modify the repository/is);
		assert.match(router, /hypothetical plan.*Brainstorming/i);
	}
});
...
test("init never writes provider CI and rejects the retired --ci flag", async () => {
	const dir = tmpRepo();
	await run(["init", dir, "--tools", "claude"]);
	assert.ok(!fs.existsSync(path.join(dir, ".github")));
	assert.ok(!fs.existsSync(path.join(dir, ".gitlab")));
	const manifest = JSON.parse(fs.readFileSync(path.join(dir, ".stdd", "manifest.json"), "utf8"));
	assert.ok(!Object.hasOwn(manifest.targets, "ci"), "the manifest stops remembering CI targets");
	// The flag is gone rather than deprecated: an adopter who scripted it gets
	// a hard error naming the flag, not a silent no-op that looks installed.
	const retired = await run(["init", tmpRepo(), "--tools", "claude", "--ci", "github"]);
	assert.equal(retired.code, 1);
	assert.match(retired.stderr, /unknown flag: --ci/);
});

test("init rejects duplicate adapter selections before creating durable state", async () => {
	for (const args of [["--tools", "claude,claude"]]) {
		const dir = tmpRepo();
...
test("init rejects an inactive local override of every mandatory read-only route", async () => {
	for (const name of ["stdd-investigation", "stdd-brainstorming"]) {
		const dir = tmpRepo();
		const local = path.join(dir, ".stdd", "playbooks", "local");
		fs.mkdirSync(local, { recursive: true });
		fs.writeFileSync(
			path.join(local, `${name}.md`),
		...
		assert.equal(res.code, 1, res.stdout + res.stderr);
		assert.match(res.stderr, new RegExp(`mandatory routing skill "${name}".*inactive`, "i"));
		assert.ok(!fs.existsSync(path.join(dir, "AGENTS.md")), "preflight failure writes no router");
		assert.ok(
...
test("start-change is reserved for persisted or repository-changing action", async () => {
	const dir = tmpRepo();
	await run(["init", dir, "--tools", "codex"]);
	const skill = fs.readFileSync(
	...
	assert.match(skill, /explicit intent to\s+persist.*modify the repository/is);
	assert.match(skill, /Read-only factual diagnosis[\s\S]*stdd-investigation/);
	assert.match(skill, /hypothetical plan[\s\S]*remains\s+Brainstorming/i);
	assert.ok(
		skill.indexOf("Start Change is the action boundary") <
			skill.indexOf('stdd task start "<short change name>"'),
	);
});
